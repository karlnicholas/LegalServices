 ...
 *
 */
public class CasetextParser extends BaseParser implements Runnable {
	private final List<CasetextOpinion> clOps;

	public CasetextParser(List<CasetextOpinion> clOps, CitationStore citationStore, StatutesTitles[] statutesTitles) {
...
//		for (LoadOpinion op : clOps) {
		...
		int i;
		for (i=0; i < l; ++i) {
			CasetextOpinion op = clOps.get(i);
