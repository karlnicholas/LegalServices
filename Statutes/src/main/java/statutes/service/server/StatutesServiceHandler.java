...
public class StatutesServiceHandler {
	...
	public Mono<ServerResponse> getStatutesRoots(ServerRequest request) {
		return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(iStatutesApi.getStatutes()));
	}

	public Mono<ServerResponse> getStatutesTitles(ServerRequest request) {
		return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(iStatutesApi.getStatutesTitles()));
	}

	public Mono<ServerResponse> getStatuteHierarchy(ServerRequest request) {
		return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(iStatutesApi.getStatutesHierarchy(request.queryParam("fullFacet").get())));
	}

	public Mono<ServerResponse> getStatutesAndHierarchies(ServerRequest request) {
		return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(request.bodyToMono(statutesKeysType).map(statutesKeys->{
			// This is a section
