...
public class WebSecurityConfig {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http, ReactiveAuthenticationManager authManager) {
        return http.authorizeExchange().pathMatchers(HttpMethod.OPTIONS).permitAll().pathMatchers("/favicon.ico").permitAll().pathMatchers("/api/user/signin", "/api/user/signup").permitAll().pathMatchers("/actuator/**").permitAll().anyExchange().authenticated().and().csrf().disable().httpBasic().disable().formLogin().disable()
//			.logout(logout -> logout.requiresLogout(new PathPatternParserServerWebExchangeMatcher("/logout")))
                ...
                }).and().addFilterAt(createAuthenticationFilter(authManager), SecurityWebFiltersOrder.AUTHENTICATION).build();
    }
    ...
    AuthenticationWebFilter createAuthenticationFilter(ReactiveAuthenticationManager authManager) {
        ...
        authenticationFilter.setServerAuthenticationConverter(exchange -> Mono.justOrEmpty(getBearerToken(exchange).flatMap(token -> jwtProperties.verifyToken(token).map(signedJWT -> {
            try {
                List<String> roles = signedJWT.getJWTClaimsSet().getStringListClaim("role");
    ...
    public static Optional<String> getBearerToken(ServerWebExchange serverWebExchange) {
        ...
        if (token.length() <= BEARER.length()) return Optional.empty();
